...
namespace DB {
	...
	bool RuleService::openTable() 
	{
		...
		try {
			db =new SQLite::Database(serviceDBPath, SQLite::OPEN_READWRITE);
		}
		catch (std::exception& e)
		{
			//ConsolePrintf("exception: %s\n", e.what()); ConsoleScanf(ch, len);
			try {
				db = new SQLite::Database(serviceDBPath, SQLite::OPEN_CREATE | SQLite::OPEN_READWRITE);
			}
			catch (std::exception& e) {
				return false;
			}
